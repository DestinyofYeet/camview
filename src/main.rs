...
use std::{sync::Arc, sync::Once, thread, time};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("camview=debug".parse()?), //.add_directive("hyper=debug".parse()?)
        )
        .compact()
        .init();
    ...
    let _cleanup_task_handle = thread::Builder::new().name("cleanup".into()).spawn({
        let ui = ui.as_weak();
        move || {
            ...
            slint::invoke_from_event_loop(move || {
                slint::quit_event_loop().expect("failed to quit UI")
            })
            .ok();
        }
    });
        ...
        async move {
            loop {
                ...
                if let Some(err) = img_data.error.as_ref() {
                    let err = anyhow::format_err!("{}", err).context("camera error");
                    err_tx.send(err).await.expect("failed to send error");
                }
        ...
        spawn({
            ...
            async move {
                ...
                loop {
                    ...
                    let wtr_icon_path: std::path::PathBuf = format!(
                        "/usr/share/icons/Arc/status/symbolic/weather-{}-symbolic.svg",
                    ...
                    let fct_icon_path: std::path::PathBuf = format!(
                        "/usr/share/icons/Arc/status/symbolic/weather-{}-symbolic.svg",
                    ...
                    ui.upgrade_in_event_loop(move |ui| {
                        let wtr_icon = match slint::Image::load_from_path(&wtr_icon_path) {
                            ...
                            Err(_) => {
                                log::error!(
                                    "failed to load weather icon for code {:?}",
                        ...
                        ui.set_wtr_forecast_time(
                            forecast.time.time().format("%H:%M").to_string().into(),
                        );
                        ui.set_wtr_forecast_precipitation_propability(
                            forecast.precipitation_probability as _,
                        );
                    })
                    .ok();
                }
...
impl std::str::FromStr for Geometry {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        s.split_once('+')
            .and_then(|(size, offset)| parse_pair(size, 'x').zip(parse_pair(offset, '+')))
            .map(|((width, height), (x, y))| Self {
                width,
                height,
                x,
                y,
...
{
    let v = s.split_once(delim).and_then(|(x, y)| {
        [x, y]
            .into_iter()
            .map(T::from_str)
...
async fn update_status(
    ...
    let url_status = {
        ...
        url.query_pairs_mut()
            .append_pair("newer", timestamp.as_str());
        url
    };
...
impl WeatherCode {
    fn into_image_name(&self, is_night: bool) -> &'static str {
        match self {
            ...
            | Self::DrizzleDense
            | Self::DrizzleFreezingLight
            | Self::DrizzleFreezingDense => "showers-scattered",

            Self::RainLight
            | Self::RainModerate
            | Self::RainHeavy
            | Self::RainFreezingLight
            | Self::RainFreezingHeavy
            | Self::ShowerRainLight
            | Self::ShowerRainModerate
...
async fn update_weather(client: &reqwest::Client, loc: &Location) -> Result<WeatherData> {
    ...
    .filter_map(|(t, day, _t2m, ta, wc, pp, ws, wd)| {
        let time = DateTime::from_timestamp(t, 0).map(|utc| utc.with_timezone(&Local))?;
    ...
    unsafe {
        LOCALE_INIT.call_once(|| {
            match sys_locale::get_locale()
                ...
                .and_then(|locstr| {
                    chrono::Locale::try_from(locstr.replace('-', "_").as_str())
